...
int main() {
...

    {
        ...
        // ---- Calcul des normales par sommet par accumulation ----
        ...
        for (auto& n : perVertexNormals) {
            n = glm::normalize(n);
        }

        // ---- Buffers GPU ----
        LavaCake::Buffer posBuffer   (device, positions,        vk::BufferUsageFlagBits::eStorageBuffer);
        LavaCake::Buffer nrmBuffer   (device, perVertexNormals, vk::BufferUsageFlagBits::eStorageBuffer);
        ...
        LavaCake::GraphicsPipeline pipeline =
            LavaCake::GraphicsPipeline::Builder(device)
                .addShaderFromFile(root + "shaders/TP3/perVertexNormal.vert",
